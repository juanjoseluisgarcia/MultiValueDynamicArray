...
namespace MultivalueDynamicArray
{
    ...
    public class MultiValueDynamicArray
    {
        ...
        private static string[] SetSvm(string[] svmValues, int svm)
        {
            if (svmValues.Length < svm)
            {
                var str = new string[svm];
                svmValues.CopyTo(str, 0);
                for (var i = 0; i < svm; i++)
                {
                    str[i] = string.Empty;
                }
        ...
        private static string JoinMultiValue(char symbol, string destination, string mv)
        {
            var result = new StringBuilder();
            result.Append(destination);
            result.Append($"{symbol}{mv}");
